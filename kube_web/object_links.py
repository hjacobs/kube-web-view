...
def render_source(links):
    f = links[0]["source"]
    result = {"name": f"{f['group']} {f['name']}".strip()}
...
def render_object_links(object_link_config, cluster, resource):
    ...
    links = reduce(
        list.__add__,
        [
            render_object_link(l, cluster, box, args)
            for l in object_link_config["links"]
        ],
        [],
    )
    ...
    return reduce(
        ...
        [
            render_group(list(l))
            for _, l in groupby(links, lambda l: l["source"]["group"])
        ],
        [],
    )
